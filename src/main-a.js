...
function updateCamera(dt) {
  ...
  camY += (target - camY) * Math.min(0.06 * dt, 1);
  if (terrain) {
...
function draw(dt) {
  ...
  drawGrid();   // screen-space, no transform
  drawStars();  // screen-space with parallax offset

  // World-space: terrain, lander, particles
  ctx.save();
  ctx.translate(0, H / 2 - camY);
